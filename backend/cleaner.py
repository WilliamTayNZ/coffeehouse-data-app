...
# Returns a dictionary containing cleaned excel sheets and their summaries
def clean_excel_sheets(excel_filename):
    ...
    for sheetname in excel_file.sheet_names: 
        ...
        summary = f"{sheetname} had {df.shape[0]} entries \n"
        ...
        if num_rows_name_added > 0:
            summary += f"Filled in {num_rows_name_added} out of {num_rows_missing_name_before} missing product names\n"
        ...
        if num_rows_price_added > 0:
            summary += f"Filled in {num_rows_price_added} out of {num_rows_missing_price_before} missing price values\n"
        ...
        summary += f"Flagged {num_missing_product_name_and_price} entries missing both Product Name and Price\n"
        # These rows can still be used for some insights: eg. associating time with order type 
        
        cleaned_sheets_dict[sheetname] = (df, summary)

    return cleaned_sheets_dict